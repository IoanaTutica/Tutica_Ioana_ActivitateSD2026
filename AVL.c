...
void adaugaMasinaInArboreEchilibrat(Nod** radacina, Masina masinaNoua) {
	if (*radacina) {
		...
		int inaltimeSt = calculeazaInaltimeArbore((*radacina)->st);
		int inaltimeDr = calculeazaInaltimeArbore((*radacina)->dr);
		int gradEchilibru = inaltimeSt - inaltimeDr;

		if (gradEchilibru == 2) {
			int inaltimeStSt = calculeazaInaltimeArbore((*radacina)->st->st);
			int inaltimeStDr = calculeazaInaltimeArbore((*radacina)->st->dr);
			if (inaltimeStSt - inaltimeStDr == -1) {
				rotireStanga(&((*radacina)->st));
			}
			rotireDreapta(radacina);
		}
		else if (gradEchilibru == -2) {
			int inaltimeDrSt = calculeazaInaltimeArbore((*radacina)->dr->st);
			int inaltimeDrDr = calculeazaInaltimeArbore((*radacina)->dr->dr);
			if (inaltimeDrSt - inaltimeDrDr == 1) {
...
void afisareMasiniDinArbore(Nod* radacina) {
	printf("\nArborele afisat inordine (crescator):\n");
	afisareInordine(radacina);

	printf("\nArborele afisat preordine:\n");
	afisarePreordine(radacina);

	printf("\nArborele afisat postordine:\n");
...
int main() {
	...
	printf("Numarul total de noduri din arborele AVL: %d\n", determinaNumarNoduri(radacina));
	printf("Inaltimea noului arbore AVL: %d\n", calculeazaInaltimeArbore(radacina));
	printf("Suma preturilor tuturor masinilor: %.2f\n", calculeazaPretTotal(radacina));

	const char* sofer = "Ion";
	printf("Pret cumulat masini pentru %s: %.2f\n\n", sofer, calculeazaPretulMasinilorUnuiSofer(radacina, sofer));

	int idDeCautat = 2;
	printf("Cautare in AVL a masinii cu id-ul %d:\n", idDeCautat);
	...
	if (m.id != -1) {
		...
		printf("Masina cautata nu se afla in arbore\n");
	}
